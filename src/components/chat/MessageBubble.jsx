...
import RecipeSuggestionLoading from "./RecipeSuggestionLoading";
...
const PHASE_PHRASES = {
  ...
  recipe: ["Menyiapkan visual resep...", "Mencari gambar yang cocok..."],
  writing: [
...
function detectPhase(statusMessage) {
  ...
  if (s.includes("sibuk") || s.includes("coba lagi") || s.includes("mencoba"))
    return "retry";
  ...
  if (s.includes("tulis") || s.includes("jawab") || s.includes("berpikir"))
    return "writing";
  return "default";
}
...
export default function MessageBubble({
  ...
  const { display: rotatingStatus, phase } =
    useRotatingStatusText(statusMessage);
  ...
  return (
    ...
    >
      ...
      >
        {isUser ? (
        ...
        ) : !content && isStreaming && recipeStatus !== "loading" ? (
          <div className="py-1 space-y-3 min-w-[240px] sm:min-w-[300px]">
            ...
            <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
              ...
              >
                <Database
                  className={`w-3 h-3 ${isCheckingStock ? "animate-pulse text-[var(--color-primary)]" : ""}`}
                />
              ...
              >
                <BrainCircuit
                  className={`w-3 h-3 ${isBuildingContext ? "animate-pulse text-[var(--color-primary)]" : ""}`}
                />
              ...
              >
                <PenTool
                  className={`w-3 h-3 ${isWritingAnswer ? "animate-pulse text-[var(--color-primary)]" : ""}`}
                />
          ...
          <div>
            <TextType
              content={content || ""}
              isStreaming={isStreaming && recipeStatus !== "loading"}
            />
            {recipeSuggestion ? (
              <RecipeSuggestionCard recipe={recipeSuggestion} />
            ) : recipeStatus === "loading" ? (
              <RecipeSuggestionLoading />
            ) : recipeStatus === "failed" ? (
              <RecipeSuggestionLoading failed />
            ) : null}
          </div>
        )}
      </div>
